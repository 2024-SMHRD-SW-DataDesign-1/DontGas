...
@Component
public class JwtUtil { // 역할 : 토큰 생성, 유효한 토큰인지 확인, 사용자 정보 확인
	...
	public JwtUtil(@Value("${jwt.secret}") String secretKey,
			@Value("${jwt.expiration_time}") Long accessTokenExpTime) // application.properties에서 가져옴
	{
		this.accessTokenExpTime = accessTokenExpTime;
		byte[] keyBytes = Decoders.BASE64.decode(secretKey);
		this.key = Keys.hmacShaKeyFor(keyBytes); // HMAC 알고리즘을 적용한 Key 객체 생성
	}

	public String createAccessToken(UserDetails userDetails) { // 만들어진 Token을 반환
	...
	public String getUserId(String token) { // username(email)을 가져오기 위한 메서드
